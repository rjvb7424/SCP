...
class Personnel:
    ...
    def _generate_biography(self) -> str:
        """Simple autogenerated biography text. You can overwrite this later."""
        pronoun = "He" if self.gender == "male" else "She"
        years = (
            f"{self.years_of_service} years"
            if getattr(self, "years_of_service", 0) > 0
            else "less than a year"
        )
        return (
            f"{self.fname} {self.lname} is a {self.position.lower()} of "
            f"{self.nationality} origin. {pronoun} has served the Foundation "
            f"for {years} and currently holds {self.clearance_level} clearance."
        )
